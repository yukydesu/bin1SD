...
public class FileDePrioriteDEntiersImpl implements FileDePrioriteDEntiers {
	...
	private void pushUp(int i){ // i = index
		//TODO
		// methode recursive

		// pas obligatoire car : -1 / 2 = 0 donc pas d'erreur
		if (i == 0)
			return;

		if (arbre[i] > arbre[indiceParent(i)]) {

			int entierTemp = arbre[i];
			arbre[i] = arbre[indiceParent(i)];
			arbre[indiceParent(i)] = entierTemp;
			pushUp(indiceParent(i));
		}
	}
		// ITERATIVE

		//int indexParent = indiceParent(i);
		//int indexEntier = i;
		//while (indexEntier != 0 && arbre[indexEntier] > arbre[indexParent]) {
		//		int entierTemp = arbre[indexEntier];
		//		arbre[indexEntier] = arbre[indexParent];
		//		arbre[indexParent] = entierTemp;
		//		indexEntier = indexParent;
		//		indexParent = indiceParent(indexEntier);
		//}
		//}
	...
	public int supprimeMax(){
		...
		int max = this.arbre[0];
		this.arbre[0] = this.arbre[taille - 1];
		this.taille --;
